    ...
    <body>
		  ...
		  <div class="modal-dialog" role="document">
		    <div class="modal-content">
		      <div class="modal-header">
		        ...
		        <h4 class="modal-title" id="myModalLabel">添加容器类型</h4>
		      </div>
		      	...
		      	<form class="form-horizontal" >
		      		<fieldset>
					    ...
					    <div class="form-group" id="paraDiv_1">
						  	<label for="inputPara" class="col-sm-3 control-label">组件1</label>
						  	<div class="col-md-6">
		...
		$("#addPara").click(function(e){
        	...
        	var str = '<div class="form-group" id="paraDiv_'+(Number(i)+1)+'">'+
			  				'<label for="inputPara" class="col-sm-3 control-label">组件'+(Number(i)+1)+'</label>'+
			  				'<div class="col-md-6">'+
        ...
        var a = function(p) {
		 ...
		 function getDemandList(proId){
	        	$.ajax({
					...
					data : JSON.stringify({level:3,projectId:proId}),
					success : function(data) {
						$("#demandList").empty();
						if (data.errorcode == 1000) {
		 ...
		 function getEnvExtendedTypeCatalog(proId){
			 ...
			 $.ajax({
					...
					data : JSON.stringify({projectId:Number(proId),level:4}),
					success : function(data) {
        ...
        function getTypeInterfaceList(){
        	$.ajax({
				...
				data : JSON.stringify({limit:limit,page:page,keyword:keyword,level:5}),
				success : function(data) {
										...
										if (data1.errorcode == 1000){
											...
											for(var i=0;i<total;i++){
												if(i!=0){
													console.log("!!!!"+i);
													var str = '<div class="form-group" id="paraDiv_'+(Number(i)+1)+'">'+
									  				'<label for="inputPara" class="col-sm-3 control-label">接口类型'+(Number(i)+1)+'</label>'+
									  				'<div class="col-md-6">'+
									  					'<input type="text" class="form-control inputPara" id="inputPara_'+(Number(i)+1)+'" placeholder="输入关键字或点击右侧箭头选择" list="inputParaList_'+(Number(i)+1)+'">'+
									  					'<datalist id="inputParaList_'+(Number(i)+1)+'" class="paraList">'+
														'</datalist>'+
