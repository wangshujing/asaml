...
<html lang="zh-CN">
    ...
    <body>
		      	...
		      	<form class="form-horizontal" >
		      		<fieldset>
					    ...
					    <div class="form-group">
					     ...
					     <div class="form-group" id="eleDiv_1">
						  	<label for="element" class="col-sm-3 control-label">成员类型1</label>
						  	<div class="col-md-6">
						  		<input type="text" class="form-control element" id="element_1" placeholder="输入关键字或点击右侧箭头选择" list="elementList_1">
						  		<datalist id="elementList_1" class="eleList">
								</datalist>
						    </div>
						</div>
						<a class="btn btn-info" type="button"  id="addEle"><i class="fa fa-plus"></i></a>
					    <div class="form-group" id="paraDiv_1" style="margin-top: -30px;">
						  	<label for="inputPara" class="col-sm-3 control-label">带标签的映射类型1</label>
						  	<div class="col-md-6">
					    ...
					    <div class="form-group" style="margin-top: -30px;" id="demandDiv_1">
        ...
        $("#TypeInterfacesBar").addClass("active");
		...
		$("#project").change(function(){
			...
			if($("#project").val()!=""||$("#project").val()!=null){
				...
				getElementCatalog($("#project").val());
			}
		})
		
		$("#addEle").click(function(e){
			var i = $("#addEle").prev().attr("id").split("_")[1];
        	var str = '<div class="form-group" id="eleDiv_'+(Number(i)+1)+'">'+
			  				'<label for="element" class="col-sm-3 control-label">成员类型'+(Number(i)+1)+'</label>'+
			  				'<div class="col-md-6">'+
			  					'<input type="text" class="form-control element" id="element_'+(Number(i)+1)+'" placeholder="输入关键字或点击右侧箭头选择" list="elementList_'+(Number(i)+1)+'">'+
			  					'<datalist id="elementList_'+(Number(i)+1)+'" class="eleList">'+
								'</datalist>'+
			    			'</div>'+
			   			'</div>'; 
			   			$("#addEle").before(str);
			   			$("#elementList_"+(Number(i)+1)).append($("#elementList_1").html());
		});
		$("#addPara").click(function(e){
			var i = $("#addPara").prev().attr("id").split("_")[1];
        	var str = '<div class="form-group" id="paraDiv_'+(Number(i)+1)+'">'+
			  				'<label for="inputPara" class="col-sm-3 control-label">带标签的映射类型'+(Number(i)+1)+'</label>'+
			  				'<div class="col-md-6">'+
        ...
        var a = function(p) {
		 ...
		 function getDemandList(proId){
	        	$.ajax({
					...
					data : JSON.stringify({level:5,projectId:proId}),
					success : function(data) {
						$("#demandList_1").empty();
						if (data.errorcode == 1000) {
		 ...
		 function getEnvExtendedTypeCatalog(proId){
			 ...
			 $.ajax({
					...
					data : JSON.stringify({projectId:Number(proId),level:2}),
					success : function(data) {
						...
						if (data.errorcode == 1000) {
							...
							if(total){
								...
								for(var i=0;i<envExtendedTypes.length;i++){
									str += '<option value="'+envExtendedTypes[i].id+'" label="'+envExtendedTypes[i].typeExpression+'" />';
								}
								$(".paraList").append(str);
 ...
 function getElementCatalog(proId){
        ...
        function getTypeInterfaceList(){
        	$.ajax({
				...
				data : JSON.stringify({limit:limit,page:page,keyword:keyword,level:3}),
				success : function(data) {
						...
						if(total){
							...
							$(".editTypeInterface").click(function(){
								...
								$.ajax({
									...
									data:JSON.stringify({id:bid,level:3}),
									success:function(data1){
										if (data1.errorcode == 1000){
											...
											var elements = data1.result.info.elements;
											...
											getDemandList(TypeInterface.projectId);
											getElementCatalog($("#project").val());
       ...
       function addTypeInterfaceInfo(){
    	   var bid = $("#bid").val();
			...
			for(var i=0;i<list3.length;i++){
				if($(list3[i]).val()!=0&&$(list3[i]).val()!=null&&$(list3[i]).val()!="")
					element.push($(list3[i]).val());
			}
			var has_error = 0;
			if(!name){
				has_error = 1;
